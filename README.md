...
Update: The website is deployed via AWS cloud services. Nginx was configured to handle all requests as well as handling a reverse-proxy to redirect http to https. The site is currently running at all times as a service on the cloud server. The website can be accessed via [this link here](https://carlosricoveri.duckdns.org/).
